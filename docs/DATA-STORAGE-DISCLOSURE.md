...
**RupeeTracker — What We Store and Where**
Last updated: 2026-04-01

This document provides full transparency on every piece of data RupeeTracker stores, where it is stored, and how to delete it.
...
- **All RupeeTracker data:** Settings > Data Management > clear browser data, or run `Object.keys(localStorage).filter(k=>k.startsWith('mypf_')).forEach(k=>localStorage.removeItem(k))` in browser console
...
## 4. What RupeeTracker Does NOT Store

| Data | Where You Might Expect It | Reality |
|---|---|---|
| Cookies | Browser cookie jar | RupeeTracker uses zero cookies |
...
| IP addresses | Access logs | Railway may log IPs in infrastructure logs (standard hosting behavior), but RupeeTracker code does not access or store them |
...
|---|---|---|---|
| Use AI assistant | OpenRouter Proxy (`/api/ai`) | Portfolio context + prompt | Subject to OpenRouter's policy |
| Sync Gmail expenses | Google (Gmail API) | Search queries | Google logs API usage per their policy |
| Parse expenses with AI | OpenRouter Proxy (`/api/ai`) | Email snippets (500 char max) | Subject to OpenRouter's policy |
| Connect broker | HDFC Securities (via Node.js proxy) | OAuth token | HDFC logs API access per their policy |
...
```javascript
// View all RupeeTracker localStorage keys and sizes
...
Run these in your browser's DevTools console (F12 > Console) to audit exactly what RupeeTracker stores.
